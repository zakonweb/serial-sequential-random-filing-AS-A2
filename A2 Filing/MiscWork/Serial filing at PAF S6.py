...
def addFieldWiseSerialFile():
    ...
    try:
        ...
        sf = open("PAFStudentsField.txt", "xt")
        sf = open("PAFStudentsField.txt", "rt")
    ...
    while ID != -1:
        ...
        stID = int(sf.readline())
        while stID != '':
            stName = sf.readline()
            stClass = sf.readline()
            stFee = sf.readline()

            if stID > ID  and added is False:
                added = True
                tf.write(str(ID) + '\n')
                tf.write(Name + '\n')
                tf.write(Class + '\n')
            ...
            tf.write(stClass + '\n')
            tf.write(str(stFee) + '\n')
        stID = int(input("Enter ID (-1 for ending): "))
    ...
    os.rename("PAFStudentsFieldT.txt", "PAFStudentsField.txt")
...
opt = options()
while opt != 0:
    if opt == 1: addFieldWiseSerialFile()
